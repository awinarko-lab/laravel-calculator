...
new class extends Component
{
    ...
    public function press(string $value): void
    {
        ...
        // Prevent starting with operator other than minus or opening parenthesis
        if ($this->expression === '' && in_array($value, ['+', '*', '/', ')'])) {
            return;
        }

        // Prevent closing parenthesis without matching opening
        if ($value === ')') {
            $openCount = substr_count($this->expression, '(');
            $closeCount = substr_count($this->expression, ')');
    ...
    {
        $formatted = preg_replace('/([+\-*\/()])/', ' $1 ', $expression);
...
<div class="flex flex-col lg:flex-row gap-6 w-full max-w-3xl">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden w-full max-w-sm">
        ...
        <div class="grid grid-cols-4 gap-px bg-gray-200 dark:bg-gray-700 p-px">
            ...
            <button wire:click="press('(')" class="bg-purple-500 dark:bg-purple-600 text-white text-xl font-medium p-4 hover:bg-purple-600 dark:hover:bg-purple-700 active:bg-purple-700 transition-colors">(</button>
            <button wire:click="press(')')" class="bg-purple-500 dark:bg-purple-600 text-white text-xl font-medium p-4 hover:bg-purple-600 dark:hover:bg-purple-700 active:bg-purple-700 transition-colors">)</button>
            ...
            {{-- Row 6: = (full width) --}}
            <button wire:click="equals" class="bg-blue-600 dark:bg-blue-700 text-white text-xl font-medium p-4 col-span-4 hover:bg-blue-700 dark:hover:bg-blue-800 active:bg-blue-800 transition-colors">=</button>
        </div>
    </div>
